...
static bool priority_check(stp_port_t *p, struct stp_config *config) {
    if (p->designated_root != config->root_id)
        ...
        return p->designated_cost < config->root_path_cost;
    else if (get_switch_id(p->designated_switch) != get_switch_id(config->switch_id))
        return get_switch_id(p->designated_switch) < get_switch_id(config->switch_id);
    else
        return get_port_id(p->port_id) < get_port_id(config->port_id);
}
...
static bool can_be_designated(stp_port_t *p, stp_t *stp) {
    if (p->designated_root != stp->designated_root)
        ...
        return p->designated_cost >= stp->designated_root;
    else if (get_switch_id(p->designated_switch) != get_switch_id(stp->switch_id))
        return get_switch_id(p->designated_switch) >= get_switch_id(stp->switch_id);
    else
        return get_port_id(p->designated_port) >= get_port_id(p->port_id);
}
