...
#define function_check(state) fprintf(stdout, \
                                      state " function %s\n", \
                                      __FUNCTION__);
...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
    ...
    struct list_head *head = &tcp_established_sock_table[hash];
    struct tcp_sock * tcp = NULL;
    list_for_each_entry(tcp, head, hash_list) {
        if (tcp->sk_sip == saddr && tcp->sk_sport == sport &&
            tcp->sk_dip == daddr && tcp->sk_dport == dport) {
            return tcp;            
        }
    }
...
struct tcp_sock *tcp_sock_lookup_listen(u32 saddr, u16 sport)
{
    ...
    struct list_head *head = &tcp_listen_sock_table[hash];
    struct tcp_sock * tcp = NULL;
    list_for_each_entry(tcp, head, hash_list) {
        if ( tcp->sk_sport == sport )
            return tcp;
    }
    // fprintf(stdout, "DONE: implement this function please.\n");
    return NULL;
...
int tcp_hash(struct tcp_sock *tsk)
{
    ...
    if (tsk->state == TCP_LISTEN) {
        fprintf(stdout, "hash tsk %p into listen queue\n", tsk);
        ...
        fprintf(stdout, "hash tsk %p into established queue\n", tsk);
    }
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
    ...
    u32 dport = ntohs(skaddr->port);
    rt_entry_t *rt = longest_prefix_match(daddr);
    if (!rt){
        return -1;
    }

    u32 saddr = rt->iface->ip;
    u32 sport = tcp_get_port();
    tsk->sk_sip = saddr;
    tsk->sk_sport = sport;
    tsk->sk_dip = daddr;
    tsk->sk_dport = dport;

    // 2. hash tcp_sock into bind_table
    tcp_bind_hash(tsk);

    // 3. control message, state switching, sleep on wait_connect
    tcp_send_control_packet(tsk, TCP_SYN);
    tcp_set_state(tsk, TCP_SYN_SENT);
    tcp_hash(tsk);

    fprintf(stdout, "tsk %p will sleep on wait_connect\n", tsk);
...
struct tcp_sock *tcp_sock_accept(struct tcp_sock *tsk)
{
    if ( tcp_sock_accept_queue_empty(tsk) ) {
        fprintf(stdout, "waiting for incoming connection request\n");
        fprintf(stdout, "tsk %p will sleep on wait_accept\n", tsk);
        if ( sleep_on(tsk->wait_accept) != 0) {
            return NULL;
        }
    }
    // fprintf(stdout, "DONE: implement this function please.\n");
    struct tcp_sock *tmpsk = tcp_sock_accept_dequeue(tsk);
    if (tcp_hash(tsk) == 0) {
        return tmpsk;
    }
    return NULL;
...
int tcp_sock_read(struct tcp_sock *tsk, char *buf, int len) {
    ...
    int read_size = min(len, tsk->rcv_wnd);
    read_ring_buffer(tsk->rcv_buf, buf, read_size);
    tsk->rcv_wnd = nused + read_size;
...
int tcp_send_data(struct tcp_sock *tsk, char *buf, int len) {
    if (!buf) {
        fprintf(stdout, "tcp_send_data: send null packet\n");
        return -1;
    }

    if (len <= 0) {
        fprintf(stdout, "tcp_send_data: data length zero or negative\n");
        return -1;
    }
    int header_size = ETHER_HDR_SIZE + IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE;
    int max_size = 1500 - header_size;
    int data_size = min(len,max_size);
    char *packet = (char*)malloc(header_size + data_size);
    memcpy(packet + header_size,
           buf,
           len);
    tcp_send_packet(tsk, packet, header_size + data_size);
    return len;
}
